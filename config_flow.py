...
def _base_schema(defaults: dict[str, Any]) -> vol.Schema:
    ...
    schema: dict[Any, Any] = {
            ...
            vol.Required(
                CONF_AI_PROVIDER, default=provider_default
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=AI_PROVIDER_OPTIONS, mode=selector.SelectSelectorMode.DROPDOWN
                )
            ),
            vol.Optional(
                CONF_OLLAMA_HOST,
...
class AiVarmeStyringConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None):
        ...
        if user_input is not None:
            errors = _validate_provider_input(user_input)
            if not errors:
        ...
        return self.async_show_form(
            step_id="user",
            data_schema=_base_schema({CONF_NAME: DEFAULT_NAME}),
            errors=errors,
        )
...
class AiVarmeStyringOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None):
        ...
        actions = ["add_room", "edit_room", "remove_room", "save"]
        ...
        schema = _base_schema(self._working_base).extend(
            {
